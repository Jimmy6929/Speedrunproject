...
import useInvoiceStore, { Invoice, InvoiceItem } from '@/utils/invoiceStore';
...
export default function EditInvoice({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    if (invoice) {
      ...
      setDueDateValue(invoice.dueDate);
      setItems(
  ...
  const addItem = () => {
    setItems([
      ...items,
      { id: uuidv4(), description: '', quantity: 1, price: 0, unitPrice: 0, amount: 0 }
    ]);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedInvoice: Invoice = {
      ...
      items: items.map(({ id: itemId, price, ...rest }) => ({
        ...rest,
        unitPrice: price
      })),
      subtotal,
      ...
      createdAt: getInvoice(id)?.createdAt || new Date().toISOString(),
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Edit Invoice {id}</h1>

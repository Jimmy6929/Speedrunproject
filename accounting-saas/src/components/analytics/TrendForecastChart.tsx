...
export default function TrendForecastChart({ 
  ...
  // Prepare data for monthly trend chart with forecast
  const getMonthlySummaryWithForecast = () => {
    if (!forecastData.hasForecast) {
      return {
        labels: [],
        revenue: [],
        expenses: [],
    ...
    // Extract data from historical and forecast
    const labels = [
      ...forecastData.historicalData.map((data) => data.month),
      ...forecastData.forecasts.map((data) => data.month + '*')
    ];
    
    const revenue = forecastData.historicalData.map((data) => data.revenue);
    const expenses = Array(forecastData.historicalData.length).fill(forecastData.avgExpense);
    const forecast = [
      ...Array(forecastData.historicalData.length).fill(null),
      ...forecastData.forecasts.map((data) => data.revenue)
    ];
    
    return { labels, revenue, expenses, forecast };
  };
  
  const monthlySummary = getMonthlySummaryWithForecast();
  
  // Chart data
  const trendWithForecastData = {
      ...
      {forecastData.hasForecast ? (
        ...
        <div className="flex flex-col items-center justify-center h-64 border border-dashed border-gray-300 dark:border-gray-600 rounded-lg">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-center max-w-xs">
            {forecastData.message || 'Insufficient data for forecasting. We need at least 3 months of historical data.'}
          </p>
        </div>
      )}
    </div>
